...
export const AuthProvider = ({ children }) => {
  ...
  const check = async () => {
    try {
      ...
      const { data } = await authAPI.getCurrentUser(); // protected route, needs cookie
      dispatch({ type: types.LOGIN_SUCCESS, payload: { user: data.user } });
  
    } catch (err) {
      if (err.response?.status === 401) {
        // User not logged in — but no need to force LOGOUT
        dispatch({ type: types.SET_LOADING, payload: false });
      } else {
        // Other errors (e.g., server crash) — you can log them or show error
    ...
    try {
      ...
      if (data.requiresMFA) {
        dispatch({
          ...
          payload: { tempToken: data.tempToken },
        });
